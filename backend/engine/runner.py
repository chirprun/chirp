...
from backend.engine.context import bind_run_trace, unbind_run_trace
from backend.engine.probes import get_random_probe
from backend.engine.quota_ops import bump_quota_after_run
        ...
        for attempt in range(HTTP_MAX_ATTEMPTS):
            try:
                ...
                if attempt < HTTP_MAX_ATTEMPTS - 1:
                    logger.warning("agent_timeout_retry", url=url, attempt=attempt + 1)
                    await _backoff(attempt)
                    continue
                raise AgentRequestFailed("agent_timeout", str(exc)) from exc
            except (httpx.ConnectError, httpx.ReadError, httpx.RemoteProtocolError) as exc:
                if attempt < HTTP_MAX_ATTEMPTS - 1:
                    logger.warning("agent_connection_retry", url=url, attempt=attempt + 1)
                    await _backoff(attempt)
                    continue
...
async def _finalize_stale_and_overlap_runs(db: AsyncSession, scenario_id: str) -> Run | None:
    ...
    for old in running_rows:
        if _utc(old.started_at) < stale_before:
            ...
            logger.warning(
                ...
                started_at=old.started_at.isoformat(),
            )
    if running_rows:
        await db.commit()
    ...
    if active:
        logger.info("run_overlap_skip", scenario_id=scenario_id, active_run_id=active.id)
    return active


def _observe_run_terminal(status: str) -> None:
    try:
        from backend.observability.metrics import observe_run_completion

        observe_run_completion(status)
    except Exception:
        pass
...
async def run_scenario(
    ...
    try:
        ...
        logger.info("run_started", scenario_id=scenario_id, run_id=run.id, scenario_name=scenario.name)
        ...
        try:
            ...
            await db.commit()
            await db.refresh(run)
            _observe_run_terminal("ERROR")
            logger.info("run_completed", scenario_id=scenario_id, run_id=run.id, status="ERROR", error_code=exc.error_code)
            return run
        ...
        for assertion in scenario.assertions:
            ...
            if assertion_type == "latency_ms":
                ...
                assertion_outcomes.append(
                    evaluate_cost(input_tokens, output_tokens, float(config.get("threshold_usd", 0.05)))
                )
                ...
                logger.warning("unknown_assertion_type", assertion_type=assertion_type)
        ...
        await bump_quota_after_run(db, scenario.id, run, input_tokens, output_tokens)
        _observe_run_terminal(run.status)
        logger.info(
            "run_completed",
            scenario_id=scenario_id,
            run_id=run.id,
        ...
        if policy:
            ...
            if alert_decision.should_alert and scenario.slack_webhook_url:
                ...
                await deliver_slack_alert_recorded(
                    db,
                    run_id=run.id,
                    scenario_id=scenario.id,
                    webhook_url=scenario.slack_webhook_url,
                    text=text,
                )
